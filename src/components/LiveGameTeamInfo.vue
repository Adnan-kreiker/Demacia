...
<template>
  <section v-if="props.gameData" class="flex mt-6 flex-row overflow-x-scroll whitespace-nowrap gap-5">
    <div class="w-[250px] min-w-[250px] bg-dark-50 bg-opacity-40 p-2" v-for="participant in team(props.team)"
      :key="participant.summonerId">
      <router-link :to="{
        path: `/summoner-info/${participant.summonerName}`,
        query: { region: props.region },
      }">
        <p class="text-lg text-center pb-2 border-b-2"
          :class="props.team === 100 ? 'border-blue-400' : 'border-red-500'">
          {{ participant.summonerName }}
        </p>
      </router-link>
      <img class="mx-auto mt-2 mb-2" height="80" width="80" :src="`https://ddragon.leagueoflegends.com/cdn/${patchVersion}/img/champion/${getChampionInfoById(props.championsArray, participant.championId)?.image.full
  }
      `" />
      <div class="flex flex-row justify-center">
        <img height="38" width="42" class="object-cover -mt-1" :src="`https://ddragon.canisback.com/img/${idToRunes(
          participant.perks.perkIds[0]
        )}`" />
        <img height="32" width="32" class="w-8 h-8 mr-2 ml-1" :src="`https://ddragon.leagueoflegends.com/cdn/${patchVersion}/img/spell/${mapSpellKeyToName(
          participant.spell1Id.toString()
        )}.png`" />
        <img height="32" width="32" class="w-8 h-8" :src="`https://ddragon.leagueoflegends.com/cdn/${patchVersion}/img/spell/${mapSpellKeyToName(
      ...
      <section v-if="summonersRankedInfo(participant.summonerId)">
        <div v-if="summonersRankedInfo(participant.summonerId)?.soloQInfo">
          <section class="">
            <div class="flex gap-2 items-center flex-row">
              <img width="60" height="50" class="max-w-[200px] object-cover" :src="`/emblems/Emblem_${capitalize(
        ...
        <section class="flex flex-row flex-wrap gap-3 justify-center">
          <n-tag type="error" v-if="summonersRankedInfo(participant.summonerId)?.soloQInfo?.hotStreak">
            Hot Streak
          </n-tag>
          <n-tag type="warning" v-if="summonersRankedInfo(participant.summonerId)?.soloQInfo?.veteran">
            Veteran
          </n-tag>
          <n-tag type="default" v-if="summonersRankedInfo(participant.summonerId)?.soloQInfo?.freshBlood">
            Waking Up
          </n-tag>
        </section>
      </section>
