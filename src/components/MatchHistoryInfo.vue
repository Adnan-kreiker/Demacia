...
import { NButton, NCollapseTransition, NDivider, NSkeleton, NSpace, NTag } from 'naive-ui'
...
const { matchHistory, getMatchHistory, loading } = useMatchHistory(summonerInfo.value.puuid, region.value, start)
...
const summoner = (participants: Participant[]): Participant => {
  const participant = participants.find(
    participant =>
      participant.summonerId === summonerInfo.value.id,
  )
  return participant!
}
...
<template>
  <div>
    ...
    <section v-else class="flex flex-col gap-3 justify-center items-center">
